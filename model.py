...
# Keeping UNIQUEID in the labels DataFrame for merging
df_labels = df[['UNIQUEID'] + label_columns]
df_features = df_features[df_features['sample_id'].isin(df['UNIQUEID'])]

# Merge features and labels on 'UNIQUEID'
df_merged = pd.merge(df_features, df_labels, left_on='sample_id', right_on='UNIQUEID')

# Prepare feature matrix and labels
X = df_merged.drop(['sample_id', 'UNIQUEID'] + label_columns, axis=1)
...
# Probabilities for each label for ROC curve
